...
| `--song-length SEC` | SIFAS **노래 길이(초)**. SIFAC 클립이 더 길면 **앞부분을 잘라** 뒤쪽 SEC초만 남깁니다(춤의 끝이 노래 끝에 맞도록). 클립이 더 짧으면 그대로 둡니다 |
...
| `--target-model FBX` | **fbx/glb/bvh 전용.** 내 SIFAS 캐릭터 FBX(메시+아마튜어)에 모션을 **절대 월드 방향**으로 직접 구워 넣습니다. 내 모델에서 바로 맞게 재생됩니다. (`.anim`엔 영향 없음) |

> **GUI:** 도구상자(`sifac_gui.py`)의 **③ 리타깃 → SIFAS** 탭에서 위 옵션을
> 슬라이더·체크박스로 그대로 조절할 수 있습니다(ArmRoll 강도, 부드러움, 프레임
> 간격, **노래 길이**, 모델에 굽기, 형식 선택, 일괄 폴더 처리). `bpy`는 ② 변환
> 탭의 원클릭 설치를 그대로 씁니다.

#### 노래 길이에 맞춰 앞부분 자르기 (`--song-length`)

SIFAC 모션이 SIFAS 노래보다 길게 만들어진 경우가 있습니다. 노래 길이(초)를 주면
**앞부분(인트로)을 잘라** 뒤쪽 길이만 남깁니다 — 춤의 **끝**이 노래의 끝에 맞도록.
잘라낸 만큼 시작 시간이 0으로 다시 맞춰지고, 루트/무대 이동의 기준 프레임도 함께
이동합니다. 클립이 노래보다 짧으면 그대로 둡니다.

```bash
# 90.5초짜리 노래에 맞춰, 더 긴 SIFAC 클립의 앞부분을 자름
python3 sifac_anim_retarget.py --sifac mot_06_maki_0510.fbx \
    --out 0510.anim --song-length 90.5
```
...
- **bvh** — 모캡 텍스트. ⚠️ BVH는 본 **롤(축 비틀림)** 을 담지 못해(관절 오프셋만 저장)
  이 리그의 방향을 정확히 표현할 수 없습니다(왕복 오차 수십 도, 일부 툴은 임포트
  거부). 가능하면 **fbx/glb를 쓰세요.**

> ### ⭐ 내 모델에 바로 입히기 — `--target-model`
>
> FBX/glTF 클립은 회전을 각 **본의 로컬 축** 기준으로 저장합니다. 그런데 우리가
> 재구성한 코어 스켈레톤과 실제 SIFAS 모델은 같은 관절을 같은 위치에 두더라도 본의
> **롤(로컬 축 비틀림)** 이 다릅니다. 그래서 이름만 맞춰 로컬 회전을 그대로 복사하면
> 팔다리가 뒤틀립니다(측정 ~63°). `--target-model`을 주면 **내 캐릭터 FBX에 모션을
> 절대 월드 방향으로 직접 구워** 각 본의 로컬 값은 Blender가 풀어내므로, 모델의 본 롤과
> **무관하게**(측정 ~0°) 내 모델에서 그대로 맞게 재생됩니다. 메시까지 함께 내보내므로
> 결과 FBX/glb는 **움직이는 내 캐릭터** 자체입니다.
>
> ```bash
> python3 sifac_anim_retarget.py --sifac mot_06_maki_0510.fbx \
>     --out 0510.anim --format fbx,glb \
>     --target-model ch0004_co0019.fbx          # ← 내 SIFAS 모델
> ```
>
> `--target-model` 없이 fbx/glb를 뽑으면 **재구성 코어 스켈레톤**만 나옵니다 —
> 그 자체로는 충실하지만, 본 축이 다른 리그에 입히면 뒤틀립니다. (Unity는 `.anim`을
> 그대로 쓰면 되므로 이 과정이 필요 없습니다.)
